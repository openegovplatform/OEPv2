...
public class WorkingUnitFinderImpl extends BasePersistenceImpl implements WorkingUnitFinder{
	...
	private static final String FIND_BY_LIKENAMESHORT = WorkingUnitFinder.class.getName() + ".findByLikeNameShort";
	...
	public List<Object[]> findByLikeNameShort(String textSearch, long parentWorkingUnitId, ServiceContext serviceContext) {
		List<Object> params = new ArrayList<Object>();
		Session session = openSession();
		String sql = CustomSQLUtil.get(FIND_BY_LIKENAMESHORT);
		sql = sql.replace("[$COMPANY_FILTER$]", " AND wk.companyId = ?");
		params.add(serviceContext.getCompanyId());
		if (textSearch != null && !"".equals(textSearch)) {
			sql = sql.replace("[$NAME_FILTER$]", " AND (LOWER(wk.name) LIKE ? OR LOWER(wk.govAgencyId) LIKE ?)");
			params.add("%" + textSearch.toLowerCase() + "%");		
			params.add("%" + textSearch.toLowerCase() + "%");	
		}
		else {
		...
		SQLQuery query = session.createSQLQuery(sql);
		query.addEntity("WorkingUnit", WorkingUnitImpl.class);
		query.addScalar("parentWorkingUnitName",Type.STRING);
		//query.addScalar("localSiteName",Type.STRING);
		//query.addScalar("organizationName",Type.STRING);
		//query.addEntity("WorkingUnit", WorkingUnitImpl.class);
		if (params != null && params.size() > 0) {
			for (int index = 0; index < params.size(); index++) {
				query.setString(index, String.valueOf(params.get(index)));
	...
	public List<Object[]> findByLikeNameShort(String textSearch, long parentWorkingUnitId, int startIndex, int endIndex, ServiceContext serviceContext) {
		List<Object> params = new ArrayList<Object>();
		Session session = openSession();
		String sql = CustomSQLUtil.get(FIND_BY_LIKENAMESHORT);
		sql = sql.replace("[$COMPANY_FILTER$]", " AND wk.companyId = ?");
		...
		if (parentWorkingUnitId > 0) {
			sql = sql.replace("[$PARENT_FILTER$]", " AND wk.parentWorkingUnitId = ?");
			params.add(parentWorkingUnitId);						
		}
		else {
			sql = sql.replace("[$PARENT_FILTER$]", "");			
